...
namespace WorkflowManager.App.Features.UserWorkflows
{
    internal sealed class UserWorkflowService(IUserService userService, IWorkflowService workflowService) : IUserWorkflowService
    {
        public void CreateUserWorkflow(int workflowId, int userId)
        {
            ...
            {
                var workflow = context.Workflows.AsNoTracking()
                .Include(x => x.WorkflowStage)
                .First(x => x.Id == workflowId); 
        ...
        public List<UserWorkflowReadModel> StagesToProcess()
        {
            ...
            {
                connection.Open();
                var groups = AppManagerCore.Instance.Dictionaries.First(x => x.Name == "Grupy użytkowników").DictionaryItems;
                var currentUserGroups = groups.Where(g => AppManagerCore.Instance.CurrentUser.GroupList().Contains(g.Value)).Select(x => x.Id).ToArray();
    ...
    public interface IUserWorkflowService : IScopedService
    {
